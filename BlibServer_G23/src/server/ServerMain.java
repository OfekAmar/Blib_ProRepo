                    ...
                    case "CHECK_BOOK_AVAILABILITY":
                        if (parts.length == 2) {
                            ...
                            try {
                                ...
                                try (PreparedStatement psBook = dbConnector.getDbConnection().prepareStatement(bookExistQuery)) {
                                    ...
                                    if (rsBook.next() && rsBook.getInt("book_count") > 0) {
                                        ...
                                        try (PreparedStatement psCopy = dbConnector.getDbConnection().prepareStatement(copyCountQuery)) {
                                            psCopy.setString(1, bookCode);
                                            ResultSet rsCopy = psCopy.executeQuery();
                                            if (rsCopy.next() && rsCopy.getInt("copy_count") > 0) {
                                                client.sendToClient("Book is available: " + bookCode + " | Copies available: " + rsCopy.getInt("copy_count"));
                                            } else {
                                ...
                                try (PreparedStatement copyPs = dbConnector.getDbConnection().prepareStatement(copyQuery)) {
                                    ...
                                    if (rsCopy.next()) {
                                        ...
                                        if ("exists".equals(copyStatus)) {
                                            ...
                                            try (PreparedStatement borrowPs = dbConnector.getDbConnection().prepareStatement(borrowQuery, Statement.RETURN_GENERATED_KEYS)) {
                                                ...
                                                if (generatedKeys.next()) {
                                                    client.sendToClient("Borrow record created successfully with ID: " + generatedKeys.getInt(1));
                                                } else {
                                            ...
                                            try (PreparedStatement resPs = dbConnector.getDbConnection().prepareStatement(reservationQuery)) {
                                                ...
                                                if (rsReservation.next()) {
                                                    ...
                                                    if (subscriberId.equals(reservedSubId) && "wait".equals(reservationStatus)) {
                                                        ...
                                                        try (PreparedStatement updateResPs = dbConnector.getDbConnection().prepareStatement(updateReservationQuery)) {
                                                        ...
                                                        client.sendToClient("ERROR: This copy is reserved by another subscriber.");
                                                    }
                                                } else {
                            ...
                            try {
                                ...
                                try (PreparedStatement datePs = dbConnector.getDbConnection().prepareStatement(dateQuery)) {
                                    ...
                                    if (rsDate.next()) {
                                        ...
                                        if (returnMaxDate.isBefore(currentDate) || returnMaxDate.isAfter(currentDate.plusDays(7))) {
                                            client.sendToClient("ERROR: Borrow cannot be extended. Return date is not within 7 days of the current date.");
                                            break;
                                        }
                                    } else {
                                ...
                                try (PreparedStatement updatePs = dbConnector.getDbConnection().prepareStatement(updateQuery)) {
                                    updatePs.setString(1, borrowId);
                                    int rowsAffected = updatePs.executeUpdate();
                                    if (rowsAffected > 0) {
                                        client.sendToClient("Borrow extended successfully. 7 days added to the return max date.");
                                    } else {
